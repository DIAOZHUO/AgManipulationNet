...
MODEL = YOLO(depo_path+"/adsorp_best.pt")
...
if __name__ == '__main__':
    ...
    # plt.show()
    bound_boxes = predict_by_path(path, threshold=0.01)
    ...
    random_box = []
    for it in bound_boxes:
        if it[1] < 220 and it[5] == 0:
            random_box = it
            break

    if random_box is not None:
        atom_pt = ((random_box[0] + random_box[2]) / 2, (random_box[1] + random_box[3]) / 2)
        print(atom_pt)
    else:
